...
from multiprocessing import Array, Value, Semaphore, Queue
...
def writeBinary(file_name, date, duration, proc_file_list):
    '''
    Writes into a file the threads information
...
def createThreads(arguments, file_list, start, end):
    """
    ...
    for file in file_list:
        ...
        thread_info.update({os.getpid():[file, size[:-1]]})
        ...
        if int(NUM_THREADS) < int(NUM_FILE):
            if NUM_FILE == 1:
                 if NUM_ARGS > 1:
        ...
        elif int(NUM_THREADS) == int(NUM_FILE):
            if NUM_ARGS > 1:
                for number in range(len(result_list)):
                    division = int(number) % 2
        ...
        if "-l" in arguments:
            thread_info[os.getpid()].append(["linhas", result_list[0]])
        elif "-c" in arguments:
            thread_info[os.getpid()].append(["caracteres", result_list[0]])
        elif "-w" in arguments:
            thread_info[os.getpid()].append(["palavras", result_list[0]])
        ...
        thread_info[os.getpid()].append(time_str)

        queue_thread.put(thread_info)
...
def threadDistribuition(number_threads, file_list):
    """
    Gives the number of files that each thread is going to read
    ...
    Requires: number_threads - int, file_list - list of .txt files
    Ensures: returns a list with the distribution of the number of files
    to read per process
    """
    num_thread_list = []                         # list with the distribution for each process

    NUM_FILES = len(file_list)                   # check variables
    NUM_THRD_INT = NUM_FILES // number_threads
    NUM_THRD_TEST = NUM_FILES % number_threads
    number_Threads_perFile_int = number_threads // NUM_FILES

    if number_threads <= NUM_FILES:

        if NUM_THRD_TEST == 0:
            for number in range(number_threads):
                num_thread_list.append(NUM_THRD_INT)

        else:
            for number in range(number_threads):
                num_thread_list.append(NUM_THRD_INT)

            if sum(num_thread_list) < NUM_FILES:
                for number in range (NUM_FILES - sum(num_thread_list)):
                    num_thread_list[number] += 1
    else:
        for number in range(NUM_FILES):
            num_thread_list.append(number_Threads_perFile_int)

        if sum(num_thread_list) < number_threads:
            for number in range (number_threads - sum(num_thread_list)):
                num_thread_list[number] += 1

    return num_thread_list
...
if __name__ == "__main__":                       # Variables are declared in "__main__"
    ...
    total = Array("i", 2)
    threadNumber = Value("i", 0)
    nThreadsBiggerThanFiles = bool(Value("i", True).value)
    ...
    queue_thread = Queue()                         # Queue to store the info for each process
    ...
    for arg in arguments:                        # Gathers the option arguments (-w or -c or -l [-L])
        ...
        if arg == "-p":
            NUM_THREADES = int(arguments[arguments.index("-p") + 1])
    ...
    thread_distributionAux = threadDistribuition(NUM_THREADES, list_files)   # Calculates the distribuition of processes per file/s
    thread_distribution = Array("i", len(thread_distributionAux))   # Creates an array for shared memory to use in the read of the amount of lines necessary

    for n in range(len(thread_distributionAux)):
        thread_distribution[n] = thread_distributionAux[n]

    if NUM_THREADES > NUM_FILES:
        nThreadsBiggerThanFiles = True

        while True:
            time.sleep(2)

            for index_thread in range (thread_distribution[INITIAL_INDEX]):
                #print(threadNumber.value)
                fileLines = linesCountingAux(list_files[INITIAL_INDEX], thread_distribution[INITIAL_INDEX])
                thread_child = Thread(target = createThreads, args = (args_list, list_files[INITIAL_INDEX : INITIAL_INDEX + 1], INITIAL_INDEX_LINES, fileLines[index_thread] + INITIAL_INDEX_LINES))
                list_threads.append(thread_child)
                thread_child.start()

                INITIAL_INDEX_LINES += fileLines[index_thread]
                #print(threadNumber.value)
                if threadNumber.value == thread_distribution[INITIAL_INDEX]:
                    #print("ola")
                    threadNumber.value = 0
                    INITIAL_INDEX_LINES = 0
                    INITIAL_INDEX += 1
                    nFilesProcessed.value += 1

            if INITIAL_INDEX == len(thread_distribution):
                break

    elif NUM_THREADES == NUM_FILES:             # Depending on the number of files and processes, creates and starts them
        nThreadsBiggerThanFiles = False

        for index_thread in range (NUM_FILES):
            thread_child = Thread(target = createThreads, args = (args_list, list_files[INITIAL_INDEX : index_thread + 1], None, None,))
            list_threads.append(thread_child)
            time.sleep(2)
            thread_child.start()

            INITIAL_INDEX = index_thread + 1
            nFilesProcessed.value += 1

    elif NUM_THREADES == 1:
        nThreadsBiggerThanFiles = False
        time.sleep(2)
        createThreads(args_list, list_files, None, None)
        nFilesProcessed.value = 1

    else:
        nThreadsBiggerThanFiles = False

        for index_thread in range (NUM_THREADES):
            time.sleep(2)
            thread_child = Thread(target = createThreads, args = (args_list, list_files[INITIAL_INDEX : INITIAL_INDEX + thread_distribution[index_thread]], None, None,))
            list_threads.append(thread_child)
            thread_child.start()

            INITIAL_INDEX = thread_distribution[index_thread]
            nFilesProcessed.value += INITIAL_INDEX


    for thread in list_threads:               # Processes are joined
        thread.join()

    for i in range(queue_thread.qsize()):
        list_infos.append(queue_thread.get())
